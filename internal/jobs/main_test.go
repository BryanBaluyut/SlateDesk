...
func run(m *testing.M) int {
	...
	if _, err := rand.Read(suffix[:]); err != nil {
		log.Printf("jobs test: generate db suffix: %v", err)
		return 1
	}
	...
	if err != nil {
		log.Printf("jobs test: connect to %s: %v (is the dev Postgres running?)", baseURL, err)
		return 1
	}
	if _, err := admin.Exec(ctx, "CREATE DATABASE "+pgx.Identifier{testDBName}.Sanitize()); err != nil {
		_ = admin.Close(ctx)
		log.Printf("jobs test: create test database: %v", err)
		return 1
	}
	defer func() {
		...
		if _, err := admin.Exec(dropCtx, "DROP DATABASE "+pgx.Identifier{testDBName}.Sanitize()+" WITH (FORCE)"); err != nil {
			log.Printf("jobs test: drop test database %s: %v", testDBName, err)
		}
	...
	if err != nil {
		log.Printf("jobs test: parse DATABASE_URL: %v", err)
		return 1
	}
	...
	if err != nil {
		log.Printf("jobs test: connect to test database: %v", err)
		return 1
	}
	defer testPool.Close()

	if err := migrate.Run(ctx, testPool); err != nil {
		log.Printf("jobs test: migrate: %v", err)
		return 1
	}
